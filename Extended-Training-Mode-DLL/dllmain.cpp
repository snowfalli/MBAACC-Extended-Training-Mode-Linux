...
const DADDRESS INPUTDISPLAYTOGGLE = (dwBaseAddress + 0x001585f8);
...
void setAllKeys()
{
	...
	oLoadStateKey.setKey(*(uint8_t*)(dwBaseAddress + adP1FN2Input));
...
//extern "C" int asmDrawText(int w, int h, int x, int y, const char* text, int alpha, int shade, int layer, void* addr, int spacing, int idek, char* out); TODO
...
void __stdcall drawText(int x, int y, const char* text, int textSize = 16, DADDRESS font = adFont1)
{
...
void frameDoneCallback()
{
	...
	if(oLoadStateKey.keyDown()) {
		snprintf(pcTextToDisplay, sizeof(pcTextToDisplay), "%s %i", "LOADED SLOT", *(char*)(dwBaseAddress + adSharedSaveSlot));
	}
...
DWORD WINAPI Init(HMODULE hModule)
{
	...
	if (hModuleCaster != NULL) { // we are running with caster
		...
		dwCasterBaseAddress = ((DWORD)hModuleCaster) - 0x66380000;
		initCasterMods();
	}
	...
	/*
	 * Hooking works by first creating a dummy d3d9 device
     * We then use this to setup a trampoline hook on the functions we want
     * I believe this is the same as what it was originally, just not written in asm which probably helped
...
BOOL APIENTRY DllMain(HMODULE hinstDLL, DWORD dwReason, LPVOID lpReserved)
{
	...
	if (dwReason == DLL_PROCESS_ATTACH) {
		...
		CloseHandle(CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)Init, hinstDLL, 0, nullptr));
