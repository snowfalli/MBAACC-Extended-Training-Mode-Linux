...
RUN \
    for arch in x86; do \
        ...
        (BIN=/opt/msvc/bin/$arch . /opt/msvc/msvcenv-native.sh && \
            ...
            clang-20 --target=$TARGET_TRIPLE -ICommon -LCommon/x86 -LDetours -L/opt/msvc/kits/10/lib/*/um/$arch -ICommon/d3dx9 -LCommon/d3dx9 -lUser32 -lGdi32 -lDbgHelp -fuse-ld=lld -std=c++20 -shared -v -g -o /out/ETM-x86.dll ETM-obj-x86.o \
        ) || exit 1; \
    done
