...
ENV FILESDLL="dllmain.cpp DebugInfo.cpp DirectX.cpp DirectXHook.cpp FancyInputDisplay.cpp FancyMenu.cpp RendererModifications.cpp ReplayManager.cpp SaveState.cpp TASManager.cpp TrainingMenu.cpp"
ENV FILESEXE="Logger.cpp FrameDisplay.cpp"
ENV COMMON="CharacterData.cpp Common.cpp RawData.cpp types.cpp"
RUN \
  for arch in x86; do \
    if [ -d /opt/msvc/kits/10/lib/*/um/$arch ]; then \
      BIN=/opt/msvc/bin/$arch . /opt/msvc/msvcenv-native.sh && \
      for file in $FILESDLL; do \
      ...
      for file in $FILESEXE; do \
          clang-20 --target=$TARGET_TRIPLE -c -o /tmpe/${file%.cpp}-$arch.o MBAACC-Extended-Training-Mode/$file \
            -ICommon -ICommon/d3dx9 -LCommon/d3dx9 -fuse-ld=lld -std=c++20 -w -Wno-unknown-pragmas; \
        done && \
      ...
      clang-20 --target=$TARGET_TRIPLE \
        -ICommon -LCommon/x86 -L/opt/msvc/kits/10/lib/*/um/$arch -ICommon/d3dx9 -LCommon/x86 -LCommon/d3dx9 -IExtended-Training-Mode-DLL/resource.h -IMBAACC-Extended-Training-Mode/FrameDisplay.h \
        -lUser32 -lGdi32 -lxinput -lDbgHelp \
        -fuse-ld=lld -std=c++20 -shared -v -w -g -Wl,/SAFESEH:NO \
        -o /out/ETM-$arch.dll /tmp/*.o /tmpc/*.o /tmpe/Logger-x86.o Extended-Training-Mode-DLL/DLLAsmFuncs.obj; \
    else \
      exit 1; \
    fi; \
  done
RUN \
    for arch in x86; do \
        ...
        (BIN=/opt/msvc/bin/$arch . /opt/msvc/msvcenv-native.sh && \
            ...
            clang-20 --target=$TARGET_TRIPLE -ICommon -LCommon/x86  -L/opt/msvc/kits/10/lib/*/um/$arch -ICommon/d3dx9 -LCommon/d3dx9 -lUser32 -lGdi32 -lGlU32 -lComDlg32 -fuse-ld=lld -std=c++20 -g -v -o /out/ETM-$arch.exe ETM-$arch.o /tmpe/*.o /tmpc/*.o  \
        ) || exit 1; \
    done
