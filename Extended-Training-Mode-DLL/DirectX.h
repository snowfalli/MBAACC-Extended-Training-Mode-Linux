...
void printDirectXError(HRESULT hr) {
	...
	const char* errorDesc = "DirectX Error Description should go here but function hates me";//DXGetErrorDescriptionA(hr);
...
bool loadResource(int id, BYTE*& buffer, unsigned& bufferSize) {
	...
	if (!hResource) {
		LogInfo("couldnt find embedded resource?");
		return false;
	}
	bufferSize = SizeofResource(hModule, hResource);
	if (!bufferSize) {
		LogInfo("getting resource size failed");
		return false;
	}
	HGLOBAL hMemory = LoadResource(hModule, hResource);
	if (!hMemory) {
		LogInfo("Failed to load resource");
		return false;
	}
	buffer = (BYTE*)LockResource(hMemory); // for some reason, this lock never has to be unlocked.
	if (!buffer) {
		LogInfo("Failed to lock resource");
		return false;
	}

	LogInfo("loaded resource successfully");
...
void _initMeltyFont() {
	...
	if (FAILED(hr)) {
		LogInfo("font createtexfromfileinmem failed??");
		return;
	}

	if (!res) {
		LogInfo("failed to load melty font resource");
		return;
	}

	hr = pD3DDevice->CreateTexture(fontTexWidth, fontTexHeight, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &texture, NULL);
	if (FAILED(hr)) {
		LogInfo("font createtexture failed");
		return;
	}
	...
	hr = pD3DDevice->GetRenderTarget(0, &oldRenderTarget);
	if (FAILED(hr)) {
		LogInfo("font getrendertarget failed");
		return;
	}

	hr = texture->GetSurfaceLevel(0, &surface);
	if (FAILED(hr)) {
		LogInfo("font getsurfacelevel failed");
		return;
	}

	hr = pD3DDevice->SetRenderTarget(0, surface);
	if (FAILED(hr)) {
		LogInfo("font setrendertarget failed");
		return;
	}
	...
	if (fontBufferMelty == NULL) {
		LogInfo("font malloc failed, what are you doing");
		return;
	}
...
void TextDraw(float x, float y, float size, DWORD ARGB, const char* format, ...) {
	...
	if (fontTexture == NULL) {
		LogInfo("fontTexture was null, im not drawing");
		return;
	}
	...
	*/
	...
	if (renderTargetTex == NULL) {
		...
		if (pSwapChain != NULL) {
			...
			//hr = pD3DDevice->CreateTexture(width, height, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &renderTargetTex, NULL);
			if (FAILED(hr)) {
				printDirectXError(hr);
			}
		}
	...
	if (needClearRectsIndex != 0) {
		pD3DDevice->Clear(needClearRectsIndex, &needClearRects[0], D3DCLEAR_TARGET, D3DCOLOR_ARGB(0, 0, 0, 0), 1.0f, 0);
		needClearRectsIndex = 0;
	}*/

	static D3DRECT clearRect = { 0, 0, width, height };
	pD3DDevice->Clear(1, &clearRect, D3DCLEAR_TARGET, D3DCOLOR_ARGB(0, 0, 0, 0), 1.0f, 0);
